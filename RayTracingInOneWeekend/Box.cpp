...
Box::Box(const vec3& p0, const vec3& p1, const std::shared_ptr<Material>& material)
{
    ...
    myList->push_back(std::make_shared<XYRectangle>(p0.X(), p1.X(), p0.Y(), p1.Y(), p0.Z(), material));
    myList->push_back(std::make_shared<FlipNormalRectangle>(std::make_shared<XYRectangle>(p0.X(), p1.X(), p0.Y(), p0.Y(), p1.Z(), material)));
